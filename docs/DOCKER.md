...
3. **前端登录**：浏览器打开 `http://localhost`，用 `.env` 中的 `INITIAL_ADMIN_USERNAME` / `INITIAL_ADMIN_PASSWORD` 登录
...
`Error: port 80 already in use` —— 可以修改 `docker-compose.yml` 里 `nginx` 服务的端口映射，例如 `"8080:80"`。
